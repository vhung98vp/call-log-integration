import uuid
from .config import logger, THRESHOLDS, ES_RL_PROPERTY, ES_PHONE_PROPERTY, ES_RL_CONF, MES_PHONE_AGG, MES_PHONE_MD
...
def check_relation_by_agg(log_agg, meta_A, meta_B):
    if log_agg[MES_PHONE_AGG['total_duration']] >= THRESHOLDS['total_duration'] or \
    ...
    threshold_avg_duration_A = (meta_A['avg_duration'] 
                                    if meta_A 
                                    and meta_A[ES_PHONE_PROPERTY['total_calls']] > 2 * THRESHOLDS['total_calls'] 
...
def calc_avg_duration(metadata):
    if metadata[ES_PHONE_PROPERTY['total_calls']] > 0:
        total_calls_from = metadata[ES_PHONE_PROPERTY['total_calls']] * metadata[ES_PHONE_PROPERTY['call_from_rate']]
        total_calls_to = metadata[ES_PHONE_PROPERTY['total_calls']] - total_calls_from
        total_duration_from = metadata[ES_PHONE_PROPERTY['avg_duration_from']] * total_calls_from
        total_duration_to = metadata[ES_PHONE_PROPERTY['avg_duration_to']] * total_calls_to
        return (total_duration_from + total_duration_to) / metadata[ES_PHONE_PROPERTY['total_calls']]
    return 0
